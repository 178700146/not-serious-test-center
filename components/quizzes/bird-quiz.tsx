  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#edf3ee] text-[#183237]">
      ...
      <div className="relative mx-auto flex min-h-screen max-w-5xl flex-col px-5 py-6 sm:px-8 sm:py-8">
        ...
        {phase === 'result' && <section className="mx-auto flex w-full max-w-2xl flex-1 flex-col justify-center py-12 text-center"><div className={`rounded-[2rem] border border-[#d1ddd8] bg-gradient-to-br ${result.accent} p-7 shadow-[0_24px_60px_rgba(28,70,75,.11)] sm:p-10`}><span className="mx-auto grid size-16 place-items-center rounded-3xl bg-[#1e5360] text-[#fff5d8] shadow-lg"><Telescope className="size-8" /></span><p className="mt-7 text-sm font-black tracking-[0.16em] text-[#bf7047]">你的成绩</p><p className="mt-2 font-serif text-6xl font-black text-[#17404a]">{score}<span className="text-2xl text-[#4d6a70]"> / {gameQuestions.length}</span></p><h1 className="mt-7 font-serif text-3xl font-black leading-tight text-[#17404a] sm:text-4xl">{result.title}</h1><p className="mt-4 text-base text-[#49656a]">{result.copy}</p><Button onClick={startGame} size="lg" className="mt-9 h-12 rounded-2xl bg-[#1e5360] px-6 text-base font-bold text-[#fffbed] hover:bg-[#163f49]"><RotateCcw className="size-4" /> 再测一次</Button></div><QuizResultActions testId="bird" title="观鸟大师" score={`${score}/${gameQuestions.length}`} detail={result.title} href="/bird" /><p className="mt-6 text-xs text-[#70858a]">图片来自自然观察记录，仅供趣味识鸟。</p></section>}
